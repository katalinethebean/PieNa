...
**Mock mode.** If `OPENAI_API_KEY` or `ANTHROPIC_API_KEY` is unset, `server/index.js` and `server/routes/upload.js` fall back to `MOCK_MODE`, returning canned transcript/scores instead of calling Whisper/Claude. `GET /api/health` reports `mockMode`. Useful for local dev without burning API credits.

**AI usage, two different providers**:
- `server/routes/upload.js` — new match upload flow: OpenAI Whisper for transcription, Anthropic Claude for 6-dimension scoring (论点/表达/驳论/结构/论据/流利度), deducts one credit per analysis (optimistic-lock style balance check before recompute).
- `server/routes/review.js` — a separate review/analysis flow using OpenRouter (`MODELS` array, primary DeepSeek v3.2 with free-tier fallbacks) with speaker-isolation regex logic (`CHAIR_TRANSITION_RE`, `FREE_DEBATE_RE`, `makeSpeakerPrefixRE`) to parse multi-speaker transcripts by role/side.

Note the scoring rubric changed over time: `sessions` table has both the older 6-column set (`argument_score`, `delivery_score`, `rebuttal_score`, `structure_score`, `evidence_score`, `fluency_score`, used by upload.js) and a newer rubric (`logic_score`, `argumentation_score`, `teamwork_score`, used by review.js / accept_match_invite). Don't assume one rubric when touching sessions data — check which flow you're in.
